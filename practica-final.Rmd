...
post_pred1<-posterior_predict(model_hurricane)
ppc_stat( y = Hurricanes$deaths,yrep =post_pred1,freq = TRUE)

post_pred2<-posterior_predict(model_hurricane_2)
ppc_stat( y = Hurricanes$deaths,yrep =post_pred2,freq = TRUE)
...
model_hurricane_3 <- stan_glm(deaths~femininity, 
                      data = Hurricanes, family = neg_binomial_2,
                      )
summary(model_hurricane_3)
```
```{r}
plot_title <- ggtitle("Distribucion posterior de coeficientes")
mcmc_areas(as.matrix(model_hurricane_3),
           #pars = c("int", "femininity"),
           prob = 0.89) + plot_title

posterior_interval(model_hurricane_3,prob=.89,pars=c('(Intercept)','femininity','reciprocal_dispersion'))

post_pred<-posterior_predict(model_hurricane_3)
ppc_stat( y = Hurricanes$deaths,yrep =post_pred,freq = TRUE)
```
...
model_hurricane_4 <- stan_glm(deaths~femininity+femininity:min_pressure, 
                      data = Hurricanes, family = neg_binomial_2,
                      )
summary(model_hurricane_4)
```
