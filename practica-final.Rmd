...
variable feminidad como predictora sugiere que **existe una relación positiva y grande entre feminidad y el número de muertes causadas por un huracán**.
...
+ $\beta_O$:  el logaritmo de la cuenta esperada para el número de muertes causadas por huracanes con nombres completamente masculinos es `r toString(round(as.vector(coef(m_hurricanes_a[[1]])[1]),2))`.
+ $\beta_1$:  en promedio, un aumento en una unidad en la "feminidad" de un nombre,
está asociado con un aumento de `r toString(round((exp(as.vector(coef(m_hurricanes_a[[1]])[2]))-1)*100,2))` por ciento en el número de muertes.
...
La estimación para los modelos 4, 5 y 6 se muestran a continuación:
...
particular énfasis en una comparación con el modelo dónde se incluye `damage_norm`
sin ninguna 
...
```
Ahora vamosa a estudiar la significancia de los coeficientes del modelo 7 al 89 por ciento. Esto se muestra en la siguiente gráfica.
...
```{r}
mcmc_areas(m_hurricanes_d[[1]], prob_outer = 0.89) + ggtitle("Distribución posterior de los coeficientes del modelo 7.")
```
